...
We made this function so that it can be used in run.py, the function when called returns a bool value
if it is true then the program runs normally otherwise sys.exit("Validation ladder failed") or something yk
'''
...
def run_validation():
    ...
    for step in range(6000):
        ...
        '''Cutoff = 2000 steps: heuristics.py S2 measured the hot-start T settling time at
        1048-1386 steps across 8 seeds, so the old 1000 cutoff started averaging inside the
        cooling tail and biased T_mean ~+2.3% warm. 2000 = worst settling (~1386) x ~1.4 safety.'''
    ...
    # Aggregate verdict (run.py runs iff all flags are true)
    ...
    if all(all_flags):
        ...
        failed = [name for name, ok in zip(
            ["3rd Law", "Energy", "Thermostat", "g(r) norm", "g(r) structure"],
            all_flags) if not ok]
        print("FAILED CHECKS:", ", ".join(failed))
        return False


''' Added this so that test.py could still run independantly'''
